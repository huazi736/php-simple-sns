...
class UserWiki extends MY_Controller {
     ...
     */
     function index() {
        ...
        $this->assign('link_url', mk_url('main/index/main', array('dkcode' => $dkcode)));
        if (($this->uid == $this->action_uid)) {
        	 $time = $this->userwiki->getEduTime($this->uid);  
        	 $timeModel = $time['birthday'] .",". $time['lastPrimary'] .",". $time['earliestHighSchool'] .",".$time['lastHighSchool'] .",". $time['earliestUni'];      	     		
             $this->assign('timeModel', $timeModel);
        	$this->selfView();                   
        } else {            
     ...
     */
     function selfView() {
        ...
        $datas['isSelf'] = true;
        $today = $this->isWorkNow($this->action_uid);
...
/**
    ...
    private function getDataInfo() {
        ...
        */
        //获取自我介绍的内容
        if ($this -> isAllow('private')) {
            $datas = $this -> getIntroduction($datas);
            $datas['permission']['private'] = 1;
        } else {
            $datas['permission']['private'] = 0;
        }        
         
        if ($this -> isAllow('relative')) {
            $datas = $this -> getRelativeDataInfo($datas);
            foreach($datas['relative'] as $key=>$val){
            $member = changeTypeToFamily($val['type']);
            $datas['relative'][$key]['typeMember'] = $member;
            $user = getUserInfo($val['relativemate']);            
            $datas['relative'][$key]['username'] = $user['username'];
            $datas['relative'][$key]['dkcode'] = $user['dkcode'];
			$datas['relative'][$key]['uid'] = $user['uid'];
            $face = get_avatar($val['relativemate'], 's');
            if($face){
            	$datas['relative'][$key]['face'] = $face;
            }else{
            	$datas['relative'][$key]['face'] = "";
            }
            }            
            $datas['permission']['relative'] = 1;
        } else {
            $datas['permission']['relative'] = 0;
        }               
     ...
     */
     function getPermissionDataInfo(array $datas) {
        ...
        */
        $datas['permission_value']['relative'] = $this -> getPermission('relative');        
        if (!$datas['permission_value']['relative']['object_content']) {
            //$datas['permission_value']['project']['object_type'] = 1;
            //这个到时根据权限来动态获取，为了方便测试暂时固定
            $datas['permission_value']['relative']['object_content'] = array();
        }
     ...
     function getJobDataInfo($uid) {
        ...
        if($v['workmate'] != '[]' && $v['workmate'] != ''){
        	...
        	foreach( $cc as $val){
        		    $vv = str_replace('"', '', $val);         		           		    
        	        $user = getUserInfo($vv);
        	        $url = mk_url('main/index/profile',array('dkcode'=>$user['dkcode']));        	        
                  	if(empty($str)){
                  		$str = '<a class="name" href="'.$url.'">'.$user['username'].'</a>';
                  	    $ids .=$vv;
                  	}else{
                        $str .= '<a class="name" href="'.$url.'">'.$user['username'].'</a>';
                  	    $ids .=",".$vv; 
     ...
     */
     function getEducationDataInfo(array $datas) {
        $edu = $this -> get_school(VISTER_UID_ziliao);       
        $datas['edu'] = $edu;
        return $datas;
    }    
	...
	function getBlockTpl(){
		if (!P('blockName')) {
			$this->ajaxReturn('', L('system_exception_notice_admin'),0);
		}		
		...
		if($blockName == 'base'){//如果是基本资料需要查询各个模块的时间
			...
			/* 现在修改基本资料的限制已经去掉了。author hxm date 2012/07/10
			 * 可以编辑除名字，性别和生日以外的其他资料。
			 */
			$nowtime = time();
		...
		$file = APPPATH.'views/edit/tpl/' . P('blockName') . '.html';			
		if (is_readable($file)) {
			...
			if($blockName != 'base'){
				$content = str_replace('<!--{$smarty.const.MISC_ROOT}-->', MISC_ROOT, $contents);
			}else{
			...
			if($blockName == 'education'){
		     	$time = $this->userwiki->getEduTime($this->uid);  
        		$timeModel = $time['birthday'] .",". $time['lastPrimary'] .",". $time['earliestHighSchool'] .",".$time['lastHighSchool'] .",". $time['earliestUni'];              		
        		$content = str_replace('<!--{$timeModel}-->', $timeModel, $contents);	
		    }
		    if($blockName == 'job'){
		        $time = $this->userwiki->getUserinfo($this->uid);		        
        		$timeModel = !empty($time['birthday']) ? date("Y-m", $time['birthday']) : 0 ;              		
        		$content = str_replace('<!--{$timeModel}-->', $timeModel, $contents);
		    }
     ...
     */
     function setPermission() {    	
		...
		if($type == 'edu' || $type == 'job' || $type == 'base'){
		$this->load->model('myeditmodel','myedit');	
        ...
        if (empty($type) || empty($permission)) {
            $data['state'] = 0;
            $this->ajaxReturn($data, L('system_exception_notice_admin'),0);
        }        
		$object_id = P('object_id');		
        $this->load->model('singleaccessmodel', '_access');                             
        if ($this -> _access -> set($type, $object_id, P('permission'))) {        	                 	    	     	        	       	
			$this->updateTimeLinePermission($type, P('permission'));			 			   
            $data['state'] = 1;                           
            die($this->ajaxReturn($data, L('system_exception_notice_admin'),1));
        } else {
            $data['state'] = 0;
           die($this->ajaxReturn($data, L('system_exception_notice_admin'),0));
        }     
	...
	function updateTimeLinePermission($type, $permission){		
		...
		if('edu' == $type){
			//教育情况
			$edu_info = $this->get_school_simple($this->action_uid);
			if($edu_info){
		...
		if(!empty($fid_arr)){
			...
			foreach($fid_arr as $key => $value){
				$soap_arr[] = array('fid' => $value, 'type' => 'uinfo', 'permission' => $access_type, 'uid'=>$this->uid);
			}      		
			$result = service('Timeline')->updateTopic($soap_arr, $access_content, true);			
			return true;
		}
		return false;
	}
     ...
     function get_school($uid) {
        ...
        if ($arr) {
            foreach ($arr as $key => $value) {
                ...
                if($value['classmate']){				                    
				$classmate = explode(' ' , $value['classmate']); 
				$ids = implode(',',$classmate); 
				$value['ids'] =  $ids;
				$users = getUserInfo($classmate); 
				foreach($users as $a=>$b) {					
					$url = mk_url('main/index/profile',array('dkcode'=>$b['dkcode']));
					if(empty($str)){
						$str = '<a class="name" href="'.$url.'">'.$b['username'].'</a>';
					} else {
						$str .= '<a class="name" href="'.$url.'">'.$b['username'].'</a>';
					}  
				}	
    ...
    public function get_school_simple($uid){
        if (!$uid) {
            return false;
        }
        $arr = $this -> userwiki -> get_schoolData($uid);
	 ...
	 * 
	 */
	public function getRelativeDataInfo(array $datas){
		$datas['relative'] = $this -> get_relative(VISTER_UID_ziliao);
        return $datas;
	}
	 ...
	 * @return array / false
	 */
	function get_relative($uid = 0) {
		if(empty($uid)) {
			return false;
		}
		$arr = $this->userwiki->getFamilyMember($uid);        
		return $arr;
	}
	/**
	 * 删除家庭成员 
	...
	public function delrelative() {
		$uid = $_POST['uid'];
		$uid = intval($uid);
		if($uid < 0) die(json_encode('0'));
		$ret = $this->userwiki->DoDelRelative($this->uid,$uid);
		if($ret === '1') die(json_encode('1'));
		die(json_encode('0'));
	}
	
	public function getIntroduction(array $datas){
	$datas['introduction'] = $this -> userwiki -> getIntroduction($this->action_uid, USERS);       
	return $datas;
	}
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!empty($friends)) {
                ...
                foreach ($friends as $key => $friend) {
                    $friends[$key]['face'] = get_avatar($friend['id'], 's');
                }
                $res['data'] = $friends;
            }
        }        
     ...
     * 查看指定用户的工作里是否包含至今
     */
    protected function isWorkNow($uid){
    	$rs = $this->userwiki->getWorkNow($uid);
   		return $rs;
    }
}
