...
"-20319|-20317|-20304|-20295|-20292|-20283|-20265|-20257|-20242|-20230|-20051|-20036|-20032|-20026|-20002|-19990".
	    ...
	    $data_value_arr = explode('|', $data_value);
	    
	    //array_combine:创建一个数组，用一个数组的值作为其键名，另一个数组的值作为其值
	    //PHP_VERSION:表示PHP的版本
	    $this->source = (PHP_VERSION>='5.0') ? array_combine($data_key_arr, $data_value_arr) : $this->_array_combine($data_key_arr, $data_value_arr);
	 ...
	 * 汉字转换成拼音
 	 * @param $string 汉字
 	 * @param $code 编码方式,默认utf8，除了gb2312与gbk之外 设置为其他任意字符则为utf8编码           
	 * **/
	public function convert($string, $code='utf8'){
		/**
	     * 如果编码不是gb2312与gbk,则启用utf-8
	     */
	    if( $code!='gb2312' || $code!='gbk') $string = $this->utf8_to_gbk($string);
	...
	// 获得拼音
	private function _pinyin($num ) {
		reset($this->source);	// 指针指向第一单元
	    if($num>0 && $num<160 )
	        return chr($num);
	 ...
	 */
	private function _array_combine($arr1, $arr2) {
	 ...
	 * utf8 转  gbk 码
	 **/
	private function utf8_to_gbk($c) {
	    $string = '';
	    if($c < 0x80) {
	        $string .= $c;
	    }elseif($c < 0x800) {
