 ...
 * @author lvxinxin
 */
class PassportService extends DK_Service
{
    ...
    {
		if(empty($dkcode) || empty($data)) return false;
	...
	{
		if(empty($dkcode) || empty($info)) return false;		
	...
	public function changeEmail($now,$new,$dkcode){
    	...
    	if(!empty($flag)){
    		if($flag['isactive'] == 1) return false;
			if($flag['dkcode'] != $dkcode) return false;
    	}
		$f = $this->db->from('user_info')->where(array('email'=>$now))->get()->row_array();
		//return $f;//$this->getLastSql();
		if(is_array($f) ){
    		if($f['isactive'] == 1) return 3;
			if($f['dkcode'] != $dkcode) return 3;			
    	}
    	//$map['email'] = $now;  
		$email = array(
					'email'=>$new,
	...
	public function checkEmail($email){
		...
		return $this->db->from('user_info')->where(array('email'=>$email,'isactive'=>1))->select('email')->get()->row_array();
	}
	  ...
	  */
	   ...
	   */
	   public function get_user_info($uid,$start,$limit){
		 $sql = 'select uid,email,username,dkcode,sex from user_info where uid !=' . $uid . ' AND isactive = 1 limit ' . $start . ',' . $limit;
		 return  $this->db->query($sql)->result_array();
		...
		*/
		public function set_edit_pwd_status($time,$dkcode){
			if(empty($dkcode) || empty($time)) return false;			
			return $this->db->update('user_auth',array('editpwdtime'=>$time),array('dkcode'=>$dkcode));						
		}
		/**
		 *获取修改密码状态
		 *
