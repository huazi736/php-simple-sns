...
class CreditService extends DK_Service
{
	/**
	 * MongoDB数据库资源
	 * 
	...
	public function status($isNew = true)
	{
	...
	public function consume($credits)
	{
		...
		$credits = (int) $credits;
		$status = $this->_db->user_credits->update(array('_id' => $this->_uid), array('$inc' => array('cc' => $credits)),
							array('safe' => true));
