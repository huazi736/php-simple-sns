...
class DK_Image{
	...
	public static function getInstance($config='')
	{
        ...
        {
            self::$instance[$key] = new DK_Image($config);
        }
	...
	private function getConfig($config)
    {
        ...
        {
            $configs = include(CONFIG_PATH . 'image.php');
	 ...
	 * @return  boolean 成功返回 true 失败返回 false
	 **/
	public function resize_two($src, $dst, $dst_w, $dst_y ,$quality=85)
	{
	    $args['src']		= $src;
		$args['dst']		= $dst;
		$args['dst_w']		= $dst_w;
	 ...
	 * @return  boolean   成功返回 true 失败返回 false
	 **/
	public function resize_ratio($src, $dst, $dst_w, $dst_y, $quality=85)
	{
		$args['src']	= $src;
		$args['dst']	= $dst;
		$args['dst_w']	= $dst_w;
	 ...
	 * @return  boolean   成功返回 true 失败返回 false
	 **/
	public function crop($src , $dst , $src_x , $src_y , $dst_w , $dst_y , $quality=85)
	{
		$args['src']	= $src;
		$args['dst']	= $dst;
		$args['src_x']	= $src_x;
	 ...
	 * @return  boolean 成功返回 true 失败返回 false
	 **/
	public function crop_ratio($src, $dst, $dst_w, $dst_y, $quality=80){
		$args['src']	= $src;
		$args['dst']	= $dst;
		$args['dst_w']	= $dst_w;
		$args['dst_y']	= $dst_y;
	 ...
	 * @return  boolean 成功返回 true 失败返回 false
	 */
	function resize_ratio_gd2($args)
	{
		$src 	= $args['src'];
		$dst	= $args['dst'];
		$dst_w	= intval($args['dst_w']);
	...
	private function resize_two_imagick($args)
	{
		...
		if($flag === true){
			if($ratio_w>=1 || $ratio_h>=1){
				if($ratio_w>1 && $ratio_h<=1){
					$resize_height = floor(($src_height/2) - ($dst_y/2));
					$cmd = $this->magick_cmd_path.''.$this->magick_cmd_convert.' "'.$src.'" -strip -coalesce -resize "'.$src_width.'x'.$dst_y.'" -layers optimize "'.$dst.'"';
				}else if($ratio_h>1 && $ratio_w<=1){
					$resize_width = floor(($src_width/2) - ($dst_w/2));
					$cmd = $this->magick_cmd_path.''.$this->magick_cmd_convert.' "'.$src.'" -strip -coalesce -resize "'.$dst_w.'x'.$src_height.'" -layers optimize "'.$dst.'"';
				}else{
					$cmd = $this->magick_cmd_path.''.$this->magick_cmd_convert.' "'.$src.'" -strip -coalesce -gravity center -crop "'.$dst_w.'x'.$dst_y.'+0+0" -layers optimize "'.$dst.'"';
				}
			}else if($ratio_w>=$ratio_h){
				$resize_width	= ceil($src_width * $ratio_w);
				$resize_height	= ceil($src_height * $ratio_w);
				$cmd = $this->magick_cmd_path.''.$this->magick_cmd_convert.' "'.$src.'" -strip -coalesce -resize "'.$resize_width.'x'.$resize_height.'" -gravity center -crop "'.$dst_w.'x'.$dst_y.'+0+0" -layers optimize "'.$dst.'"';
			}else if($ratio_w<=$ratio_h){
				$resize_width	= ceil($src_width * $ratio_h);
				$resize_height	= ceil($src_height * $ratio_h);
				$cmd = $this->magick_cmd_path.''.$this->magick_cmd_convert.' "'.$src.'" -strip -coalesce -resize "'.$resize_width.'x'.$resize_height.'" -gravity center -crop "'.$dst_w.'x'.$dst_y.'+0+0" -layers optimize "'.$dst.'"';
			...
			if($ratio_w>=1 || $ratio_h>=1){
				if($ratio_w>1 && $ratio_h<=1){
					$cmd = $this->magick_cmd_path.''.$this->magick_cmd_convert.' "'.$src.'" -strip -gravity center -crop "'.$src_width.'x'.$dst_y.'+0+0" "'.$dst.'"';
				}else if($ratio_h>1 && $ratio_w<=1){
					$cmd = $this->magick_cmd_path.''.$this->magick_cmd_convert.' "'.$src.'" -strip -gravity center -crop "'.$dst_w.'x'.$src_height.'+0+0" "'.$dst.'"';
				}else{
					$cmd = $this->magick_cmd_path.''.$this->magick_cmd_convert.' "'.$src.'" -strip -gravity center -crop "'.$dst_w.'x'.$dst_y.'+0+0" "'.$dst.'"';
				}
			}else if($ratio_w>=$ratio_h){
				$resize_width	= ceil($src_width * $ratio_w);
				$resize_height	= ceil($src_height * $ratio_w);
				$cmd = $this->magick_cmd_path.''.$this->magick_cmd_convert.' "'.$src.'" -strip -resize "'.$resize_width.'x'.$resize_height.'" -gravity center -crop "'.$dst_w.'x'.$dst_y.'+0+0" "'.$dst.'"';
			}else if($ratio_w<=$ratio_h){
				$resize_width	= ceil($src_width * $ratio_h);
				$resize_height	= ceil($src_height * $ratio_h);
				$cmd = $this->magick_cmd_path.''.$this->magick_cmd_convert.' "'.$src.'" -strip -resize "'.$resize_width.'x'.$resize_height.'" -gravity center -crop "'.$dst_w.'x'.$dst_y.'+0+0" "'.$dst.'"';
