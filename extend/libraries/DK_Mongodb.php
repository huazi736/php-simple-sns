...
class DK_Mongodb
{
    ...
    public static function getInstance($config='')
    {
        ...
        {
            self::$instance[$key] = new DK_Mongodb($config);
        }
...
/**
	...
	*/
	 ...
	 */
	 ...
	 public function insert($collection = "", $data = array()) {
	 	...
	 	try
	 	{
	 		$ret = $this->db->{$collection}->insert($data, array('fsync' => TRUE));
	 		
	 		if(isset($ret['ok']) && $ret['ok'] ==1){
	 			return true;
	 		}else{
	 			return false;
	 		}
  ...
  public function delete_all($collection = "")
  {
	...
	private function connection_string($config='') 
	{
        ...
        {
            $configs = include(CONFIG_PATH . 'mongodb.php');
