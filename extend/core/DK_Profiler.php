 ...
 */
class DK_Profiler extends DK_Loader {
	...
	protected function _compile_queries()
	{
		...
		foreach (get_dbs() as $key=>$CI_object)
		{
	...
	protected function _compile_get()
	{
		...
		else
		{
			foreach ($_GET as $key => $val)
			{
				...
				if (is_array($val))
				{
					$output['&#36;_GET['. $key .']'] = "<pre>" . stripslashes(print_r($val, true)) . "</pre>";
				}
				else
				{
					$output['&#36;_GET['. $key .']'] = stripslashes($val);
				}
			}
		}
	...
	protected function _compile_post()
	{
		...
		else
		{
			foreach ($_POST as $key => $val)
			{
				...
				if (is_array($val))
				{
					$output['&#36;_POST['. $key .']'] = '<pre>'. stripslashes(print_r($val, TRUE)). '</pre>';
				}
				else
				{
					$output['&#36;_POST['. $key .']'] = stripslashes($val);
				}
			}
		}
	...
	protected function _compile_controller_info()
	{
		...
		$output = $RTR->getController()."/".$RTR->getAction();
	...
	protected function _compile_config()
	{
		...
		foreach ($this->CI->config->config as $config=>$val)
		{
			...
			$output[$config] = stripslashes($val);
		}
	...
	function _compile_userdata()
	{
		...
		{		
			foreach ($compiled_userdata as $key => $val)
			{
				...
				if (is_array($val))
				{
					$output[$key] = stripslashes(print_r($val, true));
				}
				else
				{
		...
		{
			$output = $this->CI->lang->line('profiler_no_session_data');
		}
	...
	public function run()
	{
		...
		extract($sections);
		include(EXTEND_PATH.'errors/profiler_template.php');
