	...
	<div id="ci-profiler-menu">
		...
		<?php if (isset($sections['benchmarks'])) :?>
			<a href="#" id="ci-profiler-menu-time" onclick="show('ci-profiler-benchmarks', 'ci-profiler-menu-time'); return false;">
				<span><?php echo $BM->elapsed_time('total_execution_time_start', 'total_execution_time_end') ?> s</span>
				Load Time
			</a>
...
<?php if (count($sections) > 0) : ?>
	...
	<?php if (isset($sections['console'])) :?>
		<div id="ci-profiler-console" class="ci-profiler-box" style="display: none">
				...
				<?php foreach ($sections['console']['console'] as $log) : ?>
					...
					<?php if ($log['type'] == 'log') : ?>
						<tr>
							...
							<td class="hilight"><pre><?php echo $log['data'] ?></pre></td>
							<td></td>
						</tr>
			...
			<?php else : ?>
				<table class="main">
					<tr><td><?php echo $sections['console']; ?></td></tr>
				</table>
			<?php endif; ?>
		</div>
	<?php endif; ?>
		...
		<div id="ci-profiler-benchmarks" class="ci-profiler-box" style="display: none">
			...
			<?php if (is_array($sections['benchmarks'])) : ?>
				...
				<?php foreach ($sections['benchmarks'] as $key => $val) : ?>
					<tr><td><?php echo $key ?></td><td class="hilight"><?php echo $val ?> s</td></tr>
				<?php endforeach; ?>
				</table>

			<?php else : ?>
				<table class="main">
					<tr><td><?php echo $sections['benchmarks']; ?></td></tr>
				</table>
			<?php endif; ?>
		</div>
	<?php endif; ?>
	...
	<?php if (isset($sections['queries'])) :?>
		<div id="ci-profiler-queries" class="ci-profiler-box" style="display: none">
			...
			<?php else : ?>
				<table class="main">
					<tr><td><?php echo $sections['queries']; ?></td></tr>
				</table>
			<?php endif; ?>
		</div>
	<?php endif; ?>
		...
		<div id="ci-profiler-vars" class="ci-profiler-box" style="display: none">
			...
			<?php if (isset($sections['userdata'])) :?>
					...
					<?php if (is_array($sections['userdata'])) : ?>
						...
						<?php else : ?>
						<table class="main">
							<tr><td class="hilight">Session User Data</td><td><?php echo $sections[$section]; ?></td></tr>
						</table>
					<?php endif; ?>
				<?php endif; ?>
				...
				<?php foreach ($sections['files'] as $key => $val) : ?>
					<tr>
						<td class="hilight">
							...
							<br/><span class="faded small"><?php echo str_replace(FWPHP_PATH, '', $val) ?></span>
						</td>
					</tr>
				<?php endforeach; ?>
