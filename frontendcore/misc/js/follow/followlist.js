...
var list = {
	init: function(url) {
		if(url.type){
			var relationUrl = {
				following: {visibleUrl:'main/' + CONFIG['action_dkcode'] + '/following/visibleFollowing', moreUrl:'main/' + CONFIG['action_dkcode'] + '/following/getFollowingsByPage', searchUrl:'main/' + CONFIG['action_dkcode'] + '/following/searchFollowingByUserName'},
				follower: {moreUrl:'main/' + CONFIG['action_dkcode'] + '/follower/getfollowerBypage', searchUrl:'main/' + CONFIG['action_dkcode'] + '/follower/searchFollowerByName'},
				friend: {visibleUrl:'main/' + CONFIG['action_dkcode'] + '/friend/hideFriend', moreUrl:'main/' + CONFIG['action_dkcode'] + '/friend/getFriendByPage', searchUrl:'main/' + CONFIG['action_dkcode'] + '/friend/searchFriendByName'}
			};

			url = relationUrl[url.type];
		}

		var moreUrl = (url.mk_url === false) ? url.moreUrl : mk_url(url.moreUrl);	//更多的ajax请求地址
		this.loadmore.scrollParameter.url = moreUrl;
		this.loadmore.init();
		if(url.visibleUrl) {
			var visibleUrl = (url.mk_url === false) ? url.visibleUrl : mk_url(url.visibleUrl);	//设置隐藏的ajax请求地址
			this.visible(visibleUrl);
		}
		
		if(url.searchUrl) {
			var searchUrl = (url.mk_url === false) ? url.searchUrl : mk_url(url.searchUrl);	//搜索的ajax请求地址
			this.search.init(searchUrl);
		}

		//start: 关系按钮绑定 by卜海亮 2012-07-10
		$('div.statusBox').relation();
	...
	listHtml: function(data, parent, isSelf){
		...
		for(var i = 0; i < data.length; i++){
			...
			if(true == isSelf){
				newLi += '	<s class="setHid" id="' + item.id + '"></s>';
			}
            newLi += '	</div>\
						<div class="listInfo">\
							<span class="uName">\
						...
						<div class="statusBox" uname="' + item.name + '" rel="' + item.relation + '" uid="' + item.id + '"></div>\
                    </li>';
		...
		newItemLi.find('div.statusBox').relation();
	...
	visible: function(url) {
		$('#listWrap').delegate('s.setHid', 'click', function() {
			var par = $(this).parent(),
				data = {f_uid:this.id, action_dkcode:CONFIG['action_dkcode'], visible:false};
	...
	search: {
		...
		getData: function(keywords, url) {
			...
			_data.action_dkcode = CONFIG['action_dkcode'];
