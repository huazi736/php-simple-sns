...
/****** 发表框 *******/
	//地址
	var getWebImg_url  = mk_url("event/event/replyImg"),
		distributeMsg_url = mk_url("event/event/replyMsg"),
		distributeVideo_url   = mk_url("event/event/replyVideo");
...
/****** 信息流 *******/
	...
	$('div.infoMain').scrollLoad({
		text:"查看更多↓",	
		url:mk_url("event/event/Info"),  //请求地址
	...
	$infoMain.delegate('li.dropList_del', 'click', function(){
		...
		$.confirm('删除','你确定要执行此操作吗？',function(){
			...
			var posturl;
			var data;
			posturl = mk_url("event/event/replydel");
			$.ajax({
				url : posturl,
				type : 'post',
				dataType : 'json',
