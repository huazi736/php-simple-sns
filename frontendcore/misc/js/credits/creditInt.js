...
(function(){
	var addressForm = '<table id="newAddress">' +
		...
		'</tbody></table>';	
	var addressItem = $('.addressItem'), setAddress = $('.setAddress'), ediAddress = $('.ediAddress'), delAddress = $('.delAddress'), addNewAddress = $('#addNewAddress'), verifyBtn = $('#verifyBtn'), wrapp = $('#address-table').find('tbody');
	//popUp
	...
	var def = function (){
		...
		for(var i = 0; i<len; i++){
			...
			if(priority == 1){			
				...
				ediAddress.eq(i).parents('.addressItem').find('input[type="radio"]').attr('checked','checked');
			}			
		}
	}();
	//改变背景色
					...
					success: function(msg){
						...
						if (msg.status == 1) {		
							var item = '';
								...
								item +=	'<td><span><input class="slta" name="sltAddress" value="0" type="radio" /></span></td>';
								...
								item += '<td>';		
								item += '<p time="'+ msg.data +'" class="setAddress lh20">设为首选地址</p>';	
								item += '<p time="'+ msg.data +'" class="ediAddress lh20">修改</p>'	;	
								item += '<p time="'+ msg.data +'" class="delAddress lh20">删除</p>';		
								item += '</td></tr>';		
							wrapp.append(item);
							var lastItem = wrapp.find('.addressItem').last();
								lastItem.find('input[type="radio"]').trigger('click');
				...
				success:function(msg){
					if(msg.status == 1){
						...
						if(addressItem.size()==0){
							$('#a-table-tt').after('<tr id="nodata"><td colspan="6">没有相关的地址信息，赶紧来添加吧~~~</td></tr>');
	...
	var item = $('#address-table').find('.addressItem');
	if(item.size()>0){
		$('#addNewAddress').after('<span class="fc9">最多只能保存<strong>5</strong>个有效地址</span>');
		$('.fc9').css({
			'line-height':'28px'
	...
	verifyBtn.click(function(){
		var redeemId = $('#redeemId').val(),productId = $('#productId').val(),productName = $('#productName').html();
		var radioChked = $(':radio:checked'),items = radioChked.parents('.addressItem');
		var datas = {
			redeemId:redeemId,
			productId:productId,
			uname:items.find('.uname').html(),
			province:items.find('.province').html(),
			city:items.find('.city').html(),
			area:items.find('.area').html(),
			street:items.find('.street').html(),
			mob:items.find('.mob').html(),
			area_code:items.find('.area_code').html(),
			tel:items.find('.tel').html(),
			extension:items.find('.ediAddress').attr('extension'),		
			pcode:items.find('.ediAddress').attr('pcode')
		}
			
		if(radioChked.length != 0){			
			$.djax({
				url:mk_url('credit/redeem/confirm'),
				type:'post',
				dataType:'json',
				data:datas,
				success:function(data){
					var html = '<div class="tipsBox clearfix"><div class="fl"><img src="'+ CONFIG['misc_path'] +'img/credits/succeed.png" /></div>'
								+'<div id="excInfo" class="fl"><h5 class="fcr">恭喜您,兑换成功!</h5>'
								+'<p>您已成功兑换了<strong>'+ productName +'</strong>，我们会尽快安排发货（已发放到您的账户中）</p>'
								+'</div></div>';			
					if(data.status == 1){
						$(this).popUp({
							width:450,
							title:'实物兑换',
							content:html,
							buttons:'<span id="goLog" class="popBtns blueBtn">查看兑换记录</span><span id="goUse" class="popBtns">继续使用积分</span>',
							mask:true,
							maskMode:false
						});				
					}else{
	...
	$('#goLog').live({
		click:function(){
		window.location.href = mk_url('credit/redeem/index');
		}
	});
	$('#goUse').live({
		click:function(){
			window.location.href = mk_url('credit/product/index');
		}
	});
})()
