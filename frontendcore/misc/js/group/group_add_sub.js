...
//封装函数
var group_create = function(popButton,url_get,url_search){
	var url_post = mk_url('group/subgroup/join');
	var url_group_detail = mk_url("group/subgroup/index", {'sid': ''});
	var pager = 1;
	popButton.click(function(){
		$.djax({
			data:{page:pager,gid:$("#group_id").val(),sid:$("#subgroup_id").val()},
			url:mk_url(url_get),
			...
			if (userData.length==0) {
				...
				$(this).popUp({
					...
					buttons:'<span class="popBtns blueBtn subgroup_submit" id="group_next">确定</span><span class="popBtns" id="list_close">取消</span>',
					mask:true,
					maskMode:false,
					callback:function(){
				...
				$(".subgroup_submit").on({
					"click":function(){
						...
						if(arrChecked.length<1){
							...
							//拼接用户选取的好友的ID以及群组的logo、名称数据发送给后台
							for(var i=0;i<arrChecked.length;i++){
								liCheckedList +="&uid[]=" + arrChecked[i];
							}
							var data=liCheckedList + "&gid=" 
									+ $("#group_id").val() 
									+ "&sid=" + $("#subgroup_id").val() ;
							$.djax({
								url:url_post,
								...
								success:function(data){
									if(data.status == 1){
										window.location= url_group_detail + $("#subgroup_id").val();
									}
									if (data.status == 0) {
										$.alert(data.error);
									}
...
group_create($('#subgroup_add'),'group/subgroup/groupmember','group/subgroup/searchlastbygroup');
...
$('#subgroup_add').click(function(){//这里借用望哥的followlist.js
	list_group_fiends.init({moreUrl:'group/subgroup/groupmember', searchUrl:'group/subgroup/searchlastbygroup'});
});
	...
	loadmore: {
		...
		scroll: function() {
			...
			$('.group_list').live('jsp-scroll-y',function(event,scrollPositionY,isAtTop,isAtBottom){
				...
				var data = {page: par.pager,gid:$("#group_id").val(),sid:$("#subgroup_id").val()};
					...
					$.djax({
						...
						success: function(data) {
							...
							if(userData != 0){
								par.pager++;
								for(var i = 0; i < userData.length; i++) {
									var li= '<li class="0" id="'+userData[i].id+'"><span for="checkbox'+userData[i].id+'"><input type="checkbox" id="checkbox'+userData[i].id+'"><img width="50" height="50" src="'+userData[i].avatar+'"><a href="javascript:void(0)">'+userData[i].name+'</a></span></li>'	
	...
	search: {
		...
		getData: function(keywords, url) {
			...
			var keyword = $.trim($('.group_search_input').val()),
				_data = {keyword: keyword,page:1,gid:$("#group_id").val(),sid:$("#subgroup_id").val()};
			opt.ajax = $.ajax({
				type: 'POST',			
				url: url,
				data: _data,
