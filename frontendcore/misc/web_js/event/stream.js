...
/****** 发表框 *******/
	//地址
	/*var getWebImg_url  = getpath + "c=event&m=replyImg&web_id=" + web_id,
		distributeMsg_url = getpath + "c=event&m=replyMsg&web_id=" + web_id,
		distributeVideo_url   = getpath + "c=event&m=replyVideo&web_id=" + web_id;*/
	var getWebImg_url = mk_url('wevent/event/replyImg',{web_id:CONFIG['web_id']}),
		distributeMsg_url = mk_url('wevent/event/replyMsg',{web_id:CONFIG['web_id']}),
		distributeVideo_url = mk_url('wevent/event/replyVideo',{web_id:CONFIG['web_id']});
		infoUrl = mk_url('wevent/event/Info',{web_id:CONFIG['web_id']});
		replyDelUrl = mk_url('wevent/event/replydel',{web_id:CONFIG['web_id']});
...
/* 	$('#distributeMsg textarea.shareInfoCont').bind('keyup change',function(){
	...
	if(document.getElementById("uploadVideoFlashWrap"))
	{
		$("#uploadVideoFlash").uploadify({
			"uploader":CONFIG['miscpath']+"flash/uploadify.swf",
			"script":mk_url("video/videoapi/event_video"),
			"scriptData":{
			...
			"buttonImg":CONFIG['miscpath']+"img/system/brower.gif",
			"cancelImg":CONFIG['miscpath']+"img/system/icon_close_03.png",
				...
				success: function(data){
					if(data.status == true){
						...
						$(this).popUp({
							width:450,
							title:'提示!',
							content: '<div style="padding:10px;">发布状态发生错误请稍后重试!</div><div style="padding:0 5px 10px;">'+ data.data +'</div>',
							buttons:'',
							mask:true,
							maskMode:true,
							callback:function(){}
...
//图片上传处理
	//上传图片完毕callback func
	window.sendPhotoComplete = function (str){//data改str
		//var str = eval('('+ data +')');
		if(str.status){
			inserData(str.data,true,'app');
		}else{
			alert(str.info);
		}
		$('#tokenareaList').empty();
...
//视频上传处理
	...
	function distributeVideo(_this){
		...
		if(_message == '给这段视频做些说明吧'){
			...
			$.ajax({
				type:"POST",
				url:webpath + distributeVideo_url,
				dataType:'json',
				data:(reData),
				beforeSend: function(XMLHttpRequest){
					showAnimation(_this,showWheLoadingObj);
	...
	window.videoUploadComplete = function(data){
		...
		if(str.status == 1){
			...
			$.ajax({
				type:"POST",
				url:webpath + 'web/video/index.php?c=videoapi&m=event_video_transcod',
				dataType:"json",
			...
			$(this).popUp({
				width:450,
				title:'提示!',
				content: '<div style="padding:10px">' + str.msg + '</div>',
	...
	function createVideo(_videoURL,videoClass){
			...
			/********** ckplayer 初始化***********/
			var s1=new ckplayer();
			s1.ckplayer_url = CONFIG['miscpath']+'flash/ckplayer.swf';//播放器文件名
	...
	function showAnimation(comfirmBtn,container){
		var _str = '<div class="loadingAnimation"><img src="/misc/img/system/more_loading.gif" /></div>';
...
/****** 信息流 *******/
	...
	$('div.infoMain').scrollLoad({
		text:"查看更多↓",	
		url:infoUrl,
		data:{eventid : eventId}, //请求参数
		success:function(result){	//请求成功
			if(result.status){
				var str = result.data;
				for(var i=0; i<str.length; i++){
					inserData(str[i],true,'load');
				}
			}else{
				alert(result.info);
			}
	...
	$("div.infoList").live('mouseenter mouseleave', function() {
		$(this).find('span.deleIcon').toggle().bind('mouseenter mouseleave', function() {
			//显示下拉菜单
			$(this).find('ul').toggle();
		});
	});
	...
	inserData = function (infoData,list,strMode){
		...
		str += '			<div class="info_btm"><div class="eventComment" commentObjId="'+ infoData.tid +'" pageType="web_event" action_uid="'+ infoData.action_uid + '" msgname="" msgurl=""><!-- commentEasy插件 --></div></div>';
		str += '		</div>';
		str += '	</div>';
		if(list){
			var temp = $(str);
