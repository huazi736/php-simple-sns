 ...
 * author heyuejuan
 */ 
class InterestModel extends DkModel { 
    ...
    public function get_web_category_imid($aid){
    	$sql 	= "SELECT aid , uid , name , imid , iid , eid , category_group FROM `apps_info` where aid={$aid} limit 1";
    	$result	= $this->__query($sql);
    	$arr = array();
    	foreach($result as $key=>$val){
    		$arr[] = $val['imid'];
    ...
    public function get_web_category_group($web_id){
    	$sql 	= "select  category_group from `apps_info` where aid='{$web_id}' and `display`='0' limit 1";
    	$result	= $this->__query($sql);
    	return $result;
    }
...
/***    提供给 广告组   接口    start   ***/
	...
	public function get_web_info_imid($imid ,$start=0 , $limit=0 ){
		...
		$sql 	= "select aid from `apps_info_category` where imid in ('".implode("','" , $imid)."') {$limit} ";
		$rest_arr	= $this->__query($sql);
	...
	public function get_category_goods_brand($category_group , $start=0 , $limit=30){
		$sql 	= "SELECT * FROM `goods_brand` where category_group LIKE '{$category_group}%' and display=0 limit {$start},{$limit} ";
		return $this->__query($sql);
	}
	...
	public function get_category_brand($min_eid , $start=0 , $limit=30){
		$sql 	= "SELECT * FROM `goods_brand` where eid='{$min_eid}' and display=0 limit {$start},{$limit} ";
		return $this->__query($sql);
	}
	
	
    /**
     * 获得 组合分类		(暂时只提供购物 的组合id)
     * $eid   	分类id
     * $level 	级别
     * 返回     分类的组合 		如   1_2_3002_4985
     * **/
	public function get_category_group($id , $level){
		// $this->get_category_level_name($id ,$level);
	    if ($level <= 0)	$level = 1;
	    $group	= ""; 
        if ($level == 1) {
            $result 	= $this->get_category_level_name_one($id);
            ...
            $result 	= $this->get_category_level_name_two($id);
            if(isset($result[0]['imid']) && $result[0]['imid']>0 ){
            	$group	= $result[0]['imid'].'_'.$result[0]['iid'];
            }
        } else {
            $result = $this->get_category_level_name_more($id, $level);
            if(isset($result[0]['imid']) && $result[0]['imid']>0 ){
            	if($level==3){
            		$group	= $result[0]['imid'].'_'.$result[0]['iid'].'_'.$result[0]['eid'];
            	}else if($level==4){
            		$group	= $result[0]['imid'].'_'.$result[0]['iid'].'_'.$result[0]['piid'].'_'.$result[0]['eid'];
            	}
            }
        }
		return $group;
...
/***    提供给 广告组   接口    end   ***/
	...
	function get_data_one($aid){
		$sql 	= "select * from `apps_info` where aid='{$aid}' and `display`='0' limit 1";
		$result	= $this->__query($sql);
		return $result;
	}
...
/******    层级分类   start    ******/
	...
	public function get_category_level_name_one($id){
		$sql 	= "SELECT imid as id , imid as imid , 0 as iid, 0 as eid, 0 as piid, imname as name , 1 as level , 0 as has_son FROM `interest_category_main` where imid={$id} and is_display='1' limit 1"; 
		return $this->__query($sql);
	}
	// 二级
	public function get_category_level_name_two($id){
		$sql 	= "SELECT iid as id , imid as imid , iid as iid, 0 as eid, 0 as piid, iname as name , 2 as level , 1 as has_son FROM `interest_category` where iid={$id} and is_display='1' limit 1"; 
		return $this->__query($sql);
	}
	// 四级，五级，六级...
	public function get_category_level_name_more($id,$level){
		$sql 	= "SELECT eid as id , imid as imid , iid as iid, eid as eid, piid as piid, name as name , {$level} as level , if(piid_type=0 , 1 ,piid_type) as has_son FROM `apps_entry` where eid={$id} and display=0  limit 1";
		return $this->__query($sql);
	}
...
/*********   sql 查询	end   *************/
	 ...
	 */
	 public function get_web_cover($web_id){		
		$sql = "select webcover from `apps_info` where aid = {$web_id} limit 1";
		return $this->__query($sql);
	 }
