 ...
 * Friend 好友关系模型
 */
class RelationFriendModel extends DkModel {
    ...
    public function approveFriendRequest($uid1, $uid2) {
        ...
        if (!$noticeModel->hasPostRequest($uid2, $uid1)) {
            return false;
        }
        ...
        $time = time();
        $followingModel = DKBase::import('RelationFollowing', 'relation');
        if ($followingModel->isHiddenFollowing($uid1, $uid2)) {
            $r1 = $this->redis->zAdd('friend:hidden:' . $uid1, $time, $uid2);
        } else {
            $r1 = $this->redis->zAdd('friend:' . $uid1, $time, $uid2);
        }
        if ($followingModel->isHiddenFollowing($uid2, $uid1)) {
            $r2 = $this->redis->zAdd('friend:hidden:' . $uid1, $time, $uid2);
        } else {
            $r2 = $this->redis->zAdd('friend:' . $uid2, $time, $uid1);
        }
        $r3 = $this->redis->hSet('friend:expiry:'.$uid1, $uid2, $time);
        $r4 = $this->redis->hSet('friend:expiry:'.$uid2, $uid1, $time);
        $res = $r1 && $r2 && $r3 && $r4;
        ...
        if ($res) {
            ...
            $timelineModel = DKBase::import('Timeline');
            $userModel = DKBase::import('FastUser', 'user');

            // 获取用户信息
            $currentUserinfo = $userModel->getShortInfo($uid1);
            $targetUserinfo = $userModel->getShortInfo($uid2);

            // 添加好友成功, 当前用户时间线记录操作
            $data = array(
                'uid' => $currentUserinfo['id'],
                'uname' => $currentUserinfo['name'],
                'dkcode' => $currentUserinfo['dkcode'],
                'permission' => 4,
                'from' => 5,
                'type' => 'social',
                'dateline' => $time,
            ...
            $timelineModel->addTimeline($data);

            // 目标用户时间线记录并推送该信息到目标用户的信息流
            $data = array(
                'uid' => $targetUserinfo['id'],
                'uname' => $targetUserinfo['name'],
                'dkcode' => $targetUserinfo['dkcode'],
                'permission' => 4,
                'from' => 5,
                'type' => 'social',
                'dateline' => $time,
    ...
    public function getNumOfInvalidateFriends($uid) {
        $all = $this->redis->hgetall('friend:expiry:' . $uid);
    	$num = 0;
    	$nowtime = time();
     ...
     * @param int $uid
     */
    public function invalidateFriends($uid, $self, $page=1, $limit=20, $action_uid=''){
    	$all = $this->redis->hgetall('friend:expiry:' . $uid);
    	$uids = array();
    	$nowtime = time();
    	$default_expiry = config_item('default_user_follow_expiry');
    ...
    public function deleteFriend($uid1, $uid2) {
        ...
        $r4 = $r1 || $r2;
        $res = $r3 || $r4;
        $this->redis->hdel('friend:expiry:'.$uid1, $uid2);
		$this->redis->hdel('friend:expiry:'.$uid2, $uid1);
        if ($res) {
            //clear dump keys
	 ...
	 * --------------------------------------------------------------------------------------------
	 * @author boolee 2012/8/1
	 * @param $uid   
	 * @param $to_uid
	 * @param $timestamp int
	 * @param $accesstype string
	 **/
    public function updateFriendTime( $uid, $to_uid, $timestamp = NULL, $accesstype = NULL ){
    	if(!$uid || !$to_uid || $uid == $to_uid)
    	return 0;
    	//检查是否是好友关系
		if(!in_array($to_uid, $this->redis->zrange("friend:$uid", 0, -1)))
		return false;
		$re = $this->redis->hSet('friend:expiry:' . $uid, $to_uid, $timestamp ? $timestamp : time());
		if($re === 0 )
		return true;
		return false;
    }
